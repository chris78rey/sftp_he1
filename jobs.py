...
def run_job_async(job_id: str):
    def _worker():
        try:
            ...
            update_job(job_id, status="finished", finished_at=datetime.now(timezone.utc).isoformat(), report=report, progress={"files_done": int(report.get("total_files", 0)), "files_total": int(report.get("total_files", 0)), "bytes_done": int(report.get("total_bytes", 0)), "bytes_total": int(report.get("total_bytes", 0)), "folder": "", "file": ""})
